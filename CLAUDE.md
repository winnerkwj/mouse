...
| File | Purpose |
| --- | --- |
| `mouse_analytics.py` | **Unified** recorder: click heatmap + movement distance + click/scroll/keyboard counts, auto-saved to Excel (`openpyxl`, per-step sheets). Beginner UI, global hotkeys, excludes its own-window clicks. Keyboard is **count-only** (never stores which keys/text) |
...
- **Per-step ("software stage") model:** a recording is divided into *steps* — `Ctrl+Shift+F10` (`advance_step`) finalizes the current step (builds its heatmap, stores its stats, resets the click/distance/scroll/keyboard accumulators) and re-captures the background for the next step; stop finalizes the last step.
- **Keyboard input = COUNT ONLY (privacy):** a separate `pynput.keyboard.Listener` (coexists with the `GlobalHotKeys` hotkey listener) increments `key_count` per step. It NEVER stores which key/character was pressed — no key value reaches `events`, the summary, or any file. The pressed-key object is held only transiently in `_keys_down` to dedupe OS auto-repeat (count a held key once), then discarded. Toggle via the "키보드 기록" checkbox (`kbd_var`, default on); a global keyboard hook is keylogger-class, so AV may flag it — the startup notice says so and that only counts are kept. Each step's clicks/heatmap are independent, so you get one heatmap per software screen (Alignment → Orientation → …) in a single continuous recording.
